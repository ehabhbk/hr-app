...
class EmployeesController extends Controller
{
            ...
            return response()->json([
                'data' => $employees->map(function ($emp) {
                    ...
                    return [
                        ...
                        'notes' => $emp->notes,
                        'status' => $realStatus,
                        'warnings_count' => $emp->warnings()->where('status', '!=', 'resolved')->count() ?? 0,
                        'total_advance_remaining' => $emp->advances()->where('status', 'approved')->sum('remaining_amount'),
    ...
    public function store(Request $r)
    {
        $data = $r->validate([
            ...
            'rotation_shift_ids' => 'nullable|string',
    ...
    {
        $employee = Employee::with('department', 'attendanceDevice', 'leaves', 'warningsRelation.creator', 'assets', 'incentives', 'latestEvaluation', 'advances')->findOrFail($id);
        ...
        return response()->json([
            'data' => [
                ...
                'advances' => $employee->advances->map(function($a) {
                    return [
                        'id' => $a->id,
